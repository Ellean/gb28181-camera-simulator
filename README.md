...
设备类型将影响设备在 GB28181 平台上的识别和显示。如果不指定 `device_type`，默认使用 `IPC`（网络摄像机）。

### NVR/DVR 设备特殊功能

当配置设备类型为 `NVR` 或 `DVR` 时，模拟器会自动启用以下额外功能：

- **录像文件查询**：响应 `RecordInfo` 查询，返回模拟的录像文件列表
- **多通道支持**：每个通道可以有独立的录像文件
- **时间范围查询**：支持按时间范围查询录像
- **模拟录像数据**：自动生成模拟的录像元数据（文件名、时间、大小等）

**注意**：录像回放功能（Playback INVITE）目前返回与实时视频相同的媒体流，实际应用中应根据录像文件路径推送对应的录像内容。
